...
export default function HorizontalLabelValue(props: HorizontalLabelValueProps) {
  return (
    <div className={styles.root}>
      <label className={props.labelClassName || 'omrs-type-body-regular'} style={props.labelStyles}>
        {props.label}
        {props.specialKey && <sup>{'\u002A'}</sup>}
      </label>
      <div title={props.label} className={props.valueClassName || 'omrs-type-body-regular'} style={props.valueStyles}>
        {props.value || '\u2014'}
      </div>
    </div>
  );
