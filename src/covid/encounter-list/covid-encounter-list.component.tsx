...
import styles from './covid-encounter-list.scss';
...
const CovidOverviewList: React.FC<CovidOverviewListProps> = ({ patientUuid }) => {
  ...
  const [caseReportForm, setCaseReportForm] = useState(getForm('covid', 'case-report'));

  const forceComponentUpdate = () => setCounter(counter + 1);

  const launchCaseReportForm = (form?: any) => {
    launchOHRIWorkSpace('ohri-forms-view-ext', {
      title: caseReportForm?.name,
  ...
  return (
    <>
      {isLoading ? (
          ...
          <div className={styles.widgetContainer}>
            <div className={styles.widgetHeaderContainer}>
              ...
              <OHRIFormLauncherWithIntent
                formJson={caseReportForm}
                launchForm={launchCaseReportForm}
                onChangeIntent={setCaseReportForm}
              />
            </div>
        ...
        <EmptyState
          displayText={t('covidEncounters', 'covid case report encounters')}
